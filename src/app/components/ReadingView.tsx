...
export function ReadingView({
  ...
  const renderPageEdgeLines = (
    ...
    // Keep the stack as a solid, warm paper edge. The individual page
    // separations are drawn as light gray/brown hairlines on top. Avoid
    // vertical dark gradients; those read as stains instead of paper.
    const paperBase = isLeft
      ? "linear-gradient(to right, #ddd6c8 0%, #f6f1e7 28%, #fffdf8 62%, #eee7da 100%)"
      : "linear-gradient(to left, #ddd6c8 0%, #f6f1e7 28%, #fffdf8 62%, #eee7da 100%)";

    const sideShadow = isLeft
      ? "inset -1px 0 1px rgba(80,64,42,0.12), inset 1px 0 0 rgba(255,255,255,0.82)"
      : "inset 1px 0 1px rgba(80,64,42,0.12), inset -1px 0 0 rgba(255,255,255,0.82)";
    ...
    const lines = Array.from(
      ...
      (_, index) => {
        ...
        // Very small variation keeps it organic without creating dark bands.
        const alpha = index % 5 === 0 ? 0.43 : 0.31;
        const lineWidth = index % 11 === 0 ? 0.8 : 0.52;
        ...
        return (
          <span
            ...
            style={{
              ...
              left: `${x}px`,
              width: `${lineWidth}px`,
              background: `rgba(96, 82, 59, ${alpha})`,
              boxShadow: "0.5px 0 0 rgba(255,255,255,0.62)",
            }}
            aria-hidden="true"
          />
        );
